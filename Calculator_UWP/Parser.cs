...
namespace Calculator_UWP
{
	...
	public class Parser
	{
		private readonly ArrayList FunctionList = new ArrayList(new string[] { "abs", "acos", "asin", "atan", "ceil", "cos", "cosh", "exp", "floor", "ln", "log", "sign", "sin", "sinh", "sqrt", "tan", "tanh" });
		...
		public bool Evaluate(string Expression)
		{
			try
			{
				...
				while(m.Success)
				{
					...
					{
						Expression = Expression.Replace(m.Value, "{" + m.Groups[1].Value + Solve(m.Groups[2].Value) + "}" + m.Groups[3].Value);
					}
					else
					{
						Expression = Expression.Replace(m.Value, m.Groups[1].Value + Solve(m.Groups[2].Value));
					}
				...
				// |_________|
				//
				Value = Convert.ToDouble(Solve(Expression));
				return true;
			}
			catch
			{
			...
			while(m.Success && FunctionList.IndexOf(m.Groups[1].Value.ToLower()) > -1)
			{
				switch(m.Groups[1].Value.ToLower())
				{
					...
					case "sign":
						...
						continue;
					case "sin":
						Expression = Expression.Replace(m.Groups[0].Value, Math.Sin(Factor * Convert.ToDouble(m.Groups[2].Value)).ToString());
						m = regEx.Match(Expression);
						continue;
					case "sinh":
						Expression = Expression.Replace(m.Groups[0].Value, Math.Sinh(Factor * Convert.ToDouble(m.Groups[2].Value)).ToString());
						m = regEx.Match(Expression);
						continue;
					case "sqrt":
						...
						continue;
					case "tan":
						Expression = Expression.Replace(m.Groups[0].Value, Math.Tan(Factor * Convert.ToDouble(m.Groups[2].Value)).ToString());
						m = regEx.Match(Expression);
						continue;
					case "tanh":
						Expression = Expression.Replace(m.Groups[0].Value, Math.Tanh(Factor * Convert.ToDouble(m.Groups[2].Value)).ToString());
						m = regEx.Match(Expression);
						continue;
				}
			}
